<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="styles/normalize.css">
    <link rel="stylesheet" href="styles/index.css">
    <script defer src="js/slider.js"></script>
    <title>Document</title>
</head>
<body>
    <header class="header">
        <a href="index.html" alt="logo de la compania" class="logo.img">
            <h2 class ="logo-nombre"> Nombre compañia</h2>
            </a>
            <nav>
                <a href="index.html" class="nav-link"> Inicio</a>
                <a href="catalogo.html" class="nav-link"> Catologo</a>
                <a href="contacto.html" class="nav-link"> Contacto</a>
            </nav>
        <nav class="navigation-general"></nav>
    </header>
    <main class="main-contanier">
        <section class="primary-section">
            <article class="slider-container" id="slider-left">
                <nav class="slider-nav">
                </nav>
            </article>
            <article class="contact-container" id="icon-banner">
                    <h2>Cerrajeria</h2>
                    <a class="button WhatsApp" href="#" target="_blank" rel="contato de WhatsApp">WhatsApp</a>
                    <a class="button action" href="tel:+573107579777" rel="Numero telefonico">3107579777</a>
            </article>
            <article  class="slider-container" id="slider-right">
                <nav class="slider-nav">
                </nav>
            </article>
        </section>
    </main>
    <footer class="footer">

    </footer>
</body>
</html>